...
def local_relabel(d_weight_left, d_weight_up, d_weight_right, d_weight_down,
                  d_pull_up, d_pull_right, d_pull_down, d_pull_left,
                  d_weight_frg, d_weight_bkg, d_height_r, d_height_w, d_relabel_mask, w, h):
    x, y = cuda.grid(2)
    idx, idy = cuda.threadIdx.x, cuda.threadIdx.y
    if x >= w or y >= h:
        return
    ...
    s_height = cuda.shared.array((8 + 2, 32 + 2), dtype=int32)
    s_height[1 + idy, 1 + idx] = d_height_w[y, x]

    if idx == threads_x - 1 and x < w - 1:
        s_height[idy + 1, -1] = d_height_w[y, x + 1]
    if idx == 0 and x > 0:
        s_height[idy + 1, 0] = d_height_w[y, x - 1]
    if idy == threads_y - 1 and y < h - 1:
        s_height[-1, idx + 1] = d_height_w[y + 1, x]
    if idy == 0 and y > 0:
        s_height[0, idx + 1] = d_height_w[y - 1, x]
    ...
    if 0 < x < w - 1 and 0 < y < h - 1:
        if d_weight_bkg[y, x] > 0:
            ...
            d_height_w[y, x] = min_h + 1


@cuda.jit
def pull(d_weight_left, d_weight_up, d_weight_right, d_weight_down,
         d_pull_up, d_pull_right, d_pull_down, d_pull_left,
         d_weight_frg, d_weight_bkg, d_height, d_height_r, d_relabel_mask, w, h):
    x, y = cuda.grid(2)
    if x >= w or y >= h:
        return
...
def max_flow(img, around_capacity, bkg_similar, frg_similar, m=6, k=2, test=False):
    ...
    d_height_r = np.ones(input_shape)
    d_height_w = np.ones(input_shape)
    ...
    #     print('……………………………………………………')
        ...
        # print('active', np.sum(relabel_mask == ACTIVE) / h / w)
        # print('passive', np.sum(relabel_mask == PASSIVE) / h / w)
        if counter % 10 == 0:
            d_finish[0] = False
        ...
        if counter % 2 == 0:
            push[grid_shape, block_shape](d_weight_left, d_weight_up, d_weight_right, d_weight_down,
                                          d_pull_up, d_pull_right, d_pull_down, d_pull_left,
                                          d_weight_frg, d_weight_bkg, d_height_r, w, h, d_relabel_mask)
            ...
            local_relabel[grid_shape, block_shape](d_weight_left, d_weight_up, d_weight_right, d_weight_down,
                                                   d_pull_up, d_pull_right, d_pull_down, d_pull_left,
                                                   d_weight_frg, d_weight_bkg, d_height_r, d_height_w, d_relabel_mask,
                                                   w, h)
        else:
            push[grid_shape, block_shape](d_weight_left, d_weight_up, d_weight_right, d_weight_down,
                                          d_pull_up, d_pull_right, d_pull_down, d_pull_left,
                                          d_weight_frg, d_weight_bkg, d_height_w, w, h, d_relabel_mask)

            pull[grid_shape, block_shape](d_weight_left, d_weight_up, d_weight_right, d_weight_down,
                                          d_pull_up, d_pull_right, d_pull_down, d_pull_left,
                                          d_weight_frg, d_weight_bkg, d_height_w, d_height_r, d_relabel_mask, w, h)

            local_relabel[grid_shape, block_shape](d_weight_left, d_weight_up, d_weight_right, d_weight_down,
                                                   d_pull_up, d_pull_right, d_pull_down, d_pull_left,
                                                   d_weight_frg, d_weight_bkg, d_height_w, d_height_r, d_relabel_mask,
                                                   w, h)
        counter += 1
    ...
    from_frg_bfs_init[grid_shape, block_shape](d_weight_left, d_weight_up, d_weight_right, d_weight_down,
                                               d_pull_up, d_pull_right, d_pull_down, d_pull_left,
                                               d_weight_frg, d_weight_bkg, d_height_w, d_relabel_mask, w, h, k,
...
#     }
#
#     a = max_flow(np.zeros((4, 4)), around, bkg, frg, test=True)
